"""Tony Blake's victim profile data"""

import streamlit as st


def get_tony_blake_data():
    age_metric, time_of_death, origin_location = st.columns(3)
    age_metric.metric("Age", "58")
    time_of_death.metric("Time of Death", "10.15 AM")
    origin_location.metric("Origin", "Larskpur, CA")
    ...
    with st.expander("### Coroner's Report"):
        st.caption("### Cause of Death")
        st.error(
            "Stroke, most likely due to high levels of heart palpitations and blood pressure."
        )
    ...
    with st.expander("### Tony's Medical History"):
        st.markdown(
            """
